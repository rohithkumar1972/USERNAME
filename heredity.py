...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if people[person]["mother"] is not None and people[person]["father"] is not None:
            ...
            prob_gene_mother = 0.5 if genes == 1 else 1 - PROBS["mutation"]
            prob_gene_father = 0.5 if genes == 1 else 1 - PROBS["mutation"]
            probability *= prob_gene_mother * prob_gene_father
...
def normalize(probabilities):
    for person in probabilities:
        ...
        for gene in probabilities[person]["gene"]:
            probabilities[person]["gene"][gene] /= total_gene if total_gene != 0 else 1

        total_trait = sum(probabilities[person]["trait"].values())
        for trait in probabilities[person]["trait"]:
            probabilities[person]["trait"][trait] /= total_trait if total_trait != 0 else 1
